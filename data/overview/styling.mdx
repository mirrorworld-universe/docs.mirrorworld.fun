...
Every component comprises multiple parts that can be styled. For example, the
dialog component has the following parts: content, trigger, title, and backdrop.

The `data-part` attribute can be used to select and style a component.
...
`data-*` attributes that represent the specific state. For example, an
