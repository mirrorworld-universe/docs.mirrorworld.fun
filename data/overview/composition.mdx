...
Sometimes, you may want to attach your event handlers to the elements. To do
...
Internally, Zag maps the unique id provided to each parts needed for
the widget to work.
...
`getRootNode` function is to the machine's context.

> In shadow DOM, the root node can be derived by calling
