...
### Guide user to login
Before calling some API of MirrorSDK, users must login first.
...
But you may not want to let him login every time,so you can use CheckAuthenticated API to check if he has already logged in.
According the result,you can show him a login button or hide your login button and make him enter the game directly.
...
```
Or you can fetch NFTs by another params:

- FetchNFTsByMintAddress
- FetchNFTsByCreatorAddresses
- FetchNFTsByUpdateAuthorities

You can refer more API on [API Documentation](https://docs.mirrorworld.fun/android/android-APIReference).
