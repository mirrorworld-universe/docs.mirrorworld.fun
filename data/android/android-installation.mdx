...
Create a developer account on the <span onClick = {()=> {
window.mixgather.event("developer_dashboard")
window.open("https://app.mirrorworld.fun")}}>[Developer dashboard]()</span>.
Create a project and create an API Key.

## Import SDK

_Notice: The minimum version that SDK requires is Android 4.4._

1.  Download and uncompress <span onClick={() => {
    window.mixgather.event("download_andriod_sdk");
    window.open("https://github.com/mirrorworld-universe/mirrorworld-sdk-android/releases");}}>[Mirror
    World Android SDK]()</span>.
2.  Put the mirrorsdk.aar under folder 'libs'.If there is no libs folder,you may
    create one.
    <img
      src="https://market-assets.mirrorworld.fun/docs/build.png"
      width="30%"
      height="30%"
    />
...
1.  Guide user to login When the user opens your app, you may want to know
    whether this user needs to login, you can call the following code to know
    that.
