...
# `buy_nft`
Buy a listed NFT on a given marketplace.
...
fn main() {
    ...
    let price: f64 = 0.5; // amount in marketplace settlement currency

    let buy_result: NftListing = marketplace.buy_nft(mint_address, price).await.unwrap();
}
