...
<CH.Scrollycoding>
  ...
  ### 2. Check if user needs to login.
  When a user opens your app, you may want to know whether this user has logged in before,
  instead of letting them login every time, you can call the following code to know that.

  ```csharp IsLoggedIn.cs
  MirrorSDK.IsLoggedIn((isLoggedIn) => {
    Debug.Log("If they are logged in:" + isLoggedIn);
  });
  ```
  ---
