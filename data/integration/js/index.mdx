...
To see other chain configurations, please see the
[API reference](/api-reference/js/Configuration#chain--network-configurations)
...
async function main() {
  ...
  const tokens = await mirrorworld.Solana.Wallet.fetchTokens()
  const nfts = await mirrorworld.Solana.Asset.fetchNFTsByOwnerAddresses({ owners: [user.wallet.sol_address ]})
}
