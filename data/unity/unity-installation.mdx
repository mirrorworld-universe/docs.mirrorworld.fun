...
1. One way is to download assets[Mirror World Unity SDK](https://github.com/mirrorworld-universe/mirrorworld-sdk-unity/releases/tag/v0.1.0).  
import it to your project `Assets > Import Package > Custom Package` and select the package you just downloaded.
2. Besides, you can also search for the package in the Unity Assets Store with the keyword "com.mirror.sdk" and import it.
...
- **Api Key**: Input your API key from the developer dashboard on https://app.mirrorworld.fun
- **Debug Mode**: If checking this, you could see all the flow and error notices on the console.
- **Environment**: Choose the environment you want to use.
- **Debug Email and Password**: In the pre-release beta, you can only login SDK with this function. Input your email and password at https://docs.mirrorworld.fun.
...
When the user opens your app, you may want to know whether this user needs to log in, you can call the `IsLoggedIn` method. The boolean result is returned in the callback.
```cs
