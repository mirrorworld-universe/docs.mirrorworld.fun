# Mirror World Unity SDK(v0.131.8.1.1)
...
1. One way,download assets [Mirror World Unity SDK](https://github.com/mirrorworld-universe/mirrorworld-sdk-unity/releases/tag/v0.120.0.2.2).  
...
### User get their NFTs
To get user's NFTs,you can call API 'GetNFTsOwnedByAddress' following the below code:

```c#
List<string> owners = new List<string>();
list.Add("owner1_wallet_address");
list.Add("owner2_wallet_address");

MirrorSDK.GetNFTsOwnedByAddress(owners, (res) => {
    List<SingleNFTResponseObj> = res.nfts;
    //todo: anything with these NFTs
    
});
```

Or you can fetch NFTs another way:
- FetchNFTsByMintAddress
- FetchNFTsByCreatorAddresses
- FetchNFTsByUpdateAuthorities

You can refer to these APIs on [API document](https://docs.mirrorworld.fun/unity/unity-api).

### Transfer & List
#### Transfer
If you want to transfer a NFT to another wallet,you can use this code:

```cs
string walletAddress = "some_wallet_address";
string mintAddress = "NFT_mint_address";

MirrorSDK.TransferNFT(mintAddress,walletAddress,(res)=> {
    Debug.Log("Result of tranfering:"+res):
});
```
...
- The param 'auctionHouse' means which market you want to list.
- The param 'conformation' is a confirmation with chain,you can learn it from its explanation in this document.
...
- Finalized:
If you use this, API will wait for Solana to confirm the transaction before returning the HTTP response to you.
